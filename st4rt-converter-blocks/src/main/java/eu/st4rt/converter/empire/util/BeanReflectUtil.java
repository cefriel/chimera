...
public final class BeanReflectUtil {
	...
	public static Field getXSDIDField(Class theClass) throws InvalidRdfException {
		Field aIdField = null;

		for (Field aField : getAllDeclaredFields(theClass)) {
			if (aField.getAnnotation(XmlID.class) != null) {
				if (aIdField != null) {
					throw new InvalidRdfException("Cannot have multiple id properties");
				}
				else {
		...
		if (aIdField == null && shouldInspectSuperClass(theClass)) {
			aIdField = getXSDIDField(theClass.getSuperclass());
		}
	...
	public static void set(AccessibleObject theAccessor, Object theObj, Object theValue) throws InvocationTargetException, IllegalAccessException {
		System.out.println(theAccessor+ " - "+theObj+ " - "+theValue);
		if (theAccessor instanceof Field) {
			((Field) theAccessor).set(theObj, theValue);
		}
		else if (theAccessor instanceof Method) {
	...
	public static Field getFieldByName(Set<Field> fields, String name, Class clazz) throws IllegalStateException {
		Field field = fields.stream().filter(f -> f.getName().equals(name)).findFirst().orElse(null);

		// this should not happen!! -> exception??
		if (field == null) {
			throw new IllegalStateException("Field with name {" + name + "} of class {" + clazz + "} not found.");
		}
