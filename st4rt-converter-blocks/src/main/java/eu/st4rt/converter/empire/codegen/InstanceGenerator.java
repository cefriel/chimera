...
public final class InstanceGenerator {
	...
	public synchronized static <T> Class<T> generateInstanceClass(Class<T> theInterface) throws Exception {
		...
		if (!hasMethod(aClass, "getRdfId")) {
			aClass.addMethod(CtNewMethod.make("public st4rt.convertor.empire.annotation.SupportsRdfId.RdfKey getRdfId() { return supportsId.getRdfId(); } ", aClass));
		}

		if (!hasMethod(aClass, "setRdfId")) {
			aClass.addMethod(CtNewMethod.make("public void setRdfId(st4rt.convertor.empire.annotation.SupportsRdfId.RdfKey theURI) { supportsId.setRdfId(theURI); } ", aClass));
		}		
		...
		String equalsMethodBody = 
		  ...
		  "  if (!(theObj instanceof st4rt.convertor.empire.annotation.SupportsRdfId)) return false;\n" +
		  "  if (!(mInterfaceClass.isAssignableFrom(theObj.getClass()))) return false;\n" +
		  "  return getRdfId().equals( ((st4rt.convertor.empire.annotation.SupportsRdfId) theObj).getRdfId()) && super.equals(theObj);\n" +
		  "}\n";
