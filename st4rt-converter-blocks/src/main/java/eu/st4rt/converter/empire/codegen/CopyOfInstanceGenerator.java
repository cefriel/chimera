 ...
 * @version 1.0
 */
public final class CopyOfInstanceGenerator {
	 ...
	 * No instances
	 */
	private CopyOfInstanceGenerator() {
	...
	public synchronized static <T> Class<T> generateInstanceClass(Class<T> theInterface) throws Exception {
		...
		aClass.addField(aIdField, CtField.Initializer.byExpr("new st4rt.convertor.empire.annotation.SupportsRdfIdImpl();"));
		
		if (!hasMethod(aClass, "getRdfId")) {
			aClass.addMethod(CtNewMethod.make("public st4rt.convertor.empire.SupportsRdfId.RdfKey getRdfId() { return supportsId.getRdfId(); } ", aClass));
		}

		if (!hasMethod(aClass, "setRdfId")) {
			aClass.addMethod(CtNewMethod.make("public void setRdfId(st4rt.convertor.empire.SupportsRdfId.RdfKey theURI) { supportsId.setRdfId(theURI); } ", aClass));
		}		
		
		
		if (!hasMethod(aClass, "getAllTriples")) {
			aClass.addMethod(CtNewMethod.make("public org.eclipse.rdf4j.model.Model getAllTriples() { return mAllTriples; } ", aClass));
		}
		
		if (!hasMethod(aClass, "setAllTriples")) {
			aClass.addMethod(CtNewMethod.make("public void setAllTriples(org.eclipse.rdf4j.model.Model theGraph) { mAllTriples = theGraph; } ", aClass));
		}
		
		if (!hasMethod(aClass, "getInstanceTriples")) {
			aClass.addMethod(CtNewMethod.make("public org.eclipse.rdf4j.model.Model getInstanceTriples() { return mInstanceTriples; } ", aClass));
		}

		if (!hasMethod(aClass, "setInstanceTriples")) {
			aClass.addMethod(CtNewMethod.make("public void setInstanceTriples(org.eclipse.rdf4j.model.Model theGraph) { mInstanceTriples = theGraph; } ", aClass));
		}		
		...
		String equalsMethodBody = 
		  ...
		  "  if (!(theObj instanceof st4rt.convertor.empire.SupportsRdfId)) return false;\n" +
		  "  if (!(mInterfaceClass.isAssignableFrom(theObj.getClass()))) return false;\n" +
		  "  return getRdfId().equals( ((st4rt.convertor.empire.SupportsRdfId) theObj).getRdfId()) && super.equals(theObj);\n" +
		  "}\n";
...
//	}
	...
	private static class FinderPredicate implements Predicate<Method> {
		...
		@Override
		public boolean test(final Method theValue) {
