...
public class RecordsFactory {
    ...
    private Map<String, InputStream> streamsMap;
    ...
    public RecordsFactory(Map<String, InputStream> streamsMap) {
    ...
    private List<Record> getCSVRecords(String source) throws IOException {
        if (allCSVRecords.containsKey(source)){
            ...
            try {
                CSV csv = new CSV();
                if(Utils.isStreamFile(source)) {
                	allCSVRecords.put(source, csv.get(this.streamsMap.get(source)));
                } else {
        ...
        if (!iterators.isEmpty()) {
            ...
            if (allXMLRecords.containsKey(source) && allXMLRecords.get(source).containsKey(iterator)) {
                ...
                try {
                    ...
                    if(Utils.isStreamFile(source)) {
                    	records = xml.get(this.streamsMap.get(source), iterator);
                    } else {
        ...
        if (!iterators.isEmpty()) {
            ...
            if (allJSONRecords.containsKey(source) && allJSONRecords.get(source).containsKey(iterator)) {
                ...
                try {
                    ...
                    List<Record> records = new ArrayList<>();
                    if(Utils.isStreamFile(source)) {
                    	records = json.get(this.streamsMap.get(source), iterator);
                    } else {
